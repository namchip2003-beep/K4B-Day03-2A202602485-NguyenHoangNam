...
if __name__ == "__main__":
    ...
    sched_tool = next((t for t in tools if t.get("name") == "renew_document"), None)
    if sched_tool and not sched_tool.get("parameters", {}).get("properties"):
        print("⏳ [TODO 1.2]: Tool 'renew_document' chưa được định nghĩa properties trong 'src/tools.py'.")
    else:
        print("✅ [TODO 1.2]: Tool 'renew_document' đã có schema đầy đủ.")

    # Kiểm tra trạng thái TODO 2.1 (call_tool)
    test_result = server.call_tool("library_query", {"document_id": "DOC2026001"})
    if not test_result:
